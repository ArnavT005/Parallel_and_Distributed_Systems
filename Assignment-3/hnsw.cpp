...
// double *get_vec(double *vect, int node, int start, int end, int num_lines,
// int size, int embedding_size, MPI_Datatype &vector_t, MPI_Win &win) {
...
//     MPI_Win_fence(0, win);
//     MPI_Get((void *)v, 1, vector_t, target, node - target * num_lines / size,
//     1,
//             vector_t, win);
//     MPI_Win_fence(0, win);
...
//                               V<int> &level_offset, int level, V<bool>
//                               &visited, double *vect, int K, int start_news,
//                               int end_news, int num_lines, int size, int
//                               embedding_size, MPI_Datatype &vector_t, MPI_Win
//                               &win) {
...
//       double *ep_vec = get_vec(vect, index[i], start_news, end_news,
//       num_lines,
...
V<P<int, double>> SearchLayer(double *user, V<P<int, double>> &candidates,
  ...
  while (candidates.size() > 0) {
    ...
    for (int i = start; i < end; i++) {
      // printf("Processing embedding for index[%d] = %d\n", i, index[i]);
      if (i >= 262375800) {
        printf("start: %d, end: %d, i: %d, ep: %d, level: %d\n", start, end, i,
               ep, level);
        assert(false);
      }

      assert(index[i] < 1305265);
...
// V<P<int, double>> QueryHNSW(double *user, int ep, V<int> &indptr, V<int>
// &index, V<int> &level_offset, int max_level, double *vect, int K, int start,
// int end, int num_lines, int size, int embedding_size, MPI_Datatype &vector_t,
// MPI_Win &win) {
//   V<P<int, double>> topk;
//   auto ep_vec = get_vec(vect, ep, start, end, num_lines, size,
//   embedding_size, vector_t, win); topk.push_back(P<int, double>(ep,
//   cosine_dist(user, ep_vec, embedding_size))); V<bool> visited(num_lines,
//   false); visited[ep] = true; for (int level = max_level; level >= 0;
//   level--) {
//     // printf("Calling search layer: %d\n", level);
//     topk = SearchLayer(user, topk, indptr, index, level_offset, level,
//     visited,
