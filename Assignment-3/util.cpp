...
void read_vect(std::string file_name, V<int> &vect) {
  MPI_File fin;
  if (MPI_File_open(MPI_COMM_WORLD, file_name.c_str(), MPI_MODE_RDONLY, MPI_INFO_NULL, &fin) != MPI_SUCCESS) {
  ...
  MPI_File_close(&fin);
  printf("Read vector from %s, got %ld elements\n", file_name.c_str(), vect.size());
}

void get_embedding_info(std::string file_name, uint &embedding_size, uint &num_lines) {
  MPI_File fin;
  if (MPI_File_open(MPI_COMM_WORLD, file_name.c_str(), MPI_MODE_RDONLY, MPI_INFO_NULL, &fin) != MPI_SUCCESS) {
    printf("Unable to open file: %s", file_name.c_str());
    return;
...
double* read_embeddings(std::string file_name, int rank, int size, MPI_Datatype &vector_t, uint num_lines, uint embedding_size) {
  MPI_File fin;
  if (MPI_File_open(MPI_COMM_WORLD, file_name.c_str(), MPI_MODE_RDONLY, MPI_INFO_NULL, &fin) != MPI_SUCCESS) {
    printf("Unable to open file: %s", file_name.c_str());
    return nullptr;
  ...
  printf("Starting to read %d-%d lines, with embedding size: %d\n", start, end, embedding_size);
  MPI_File_seek(fin, 2 * sizeof(uint) + start * embedding_size * sizeof(double), MPI_SEEK_SET);
  double* buff = new double[((ll) embedding_size) * (end - start)];
  for (int i = start; i < end; i++) {
    MPI_File_read(fin, &buff[(i - start) * embedding_size], 1, vector_t, MPI_STATUS_IGNORE);
  }
  MPI_File_close(&fin);
  return buff;
}