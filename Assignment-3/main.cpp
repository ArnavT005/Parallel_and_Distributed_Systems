...
int main(int argc, char **argv) {
  ...
  if (MPI_File_open(MPI_COMM_WORLD, out_file.c_str(),
  ...
  MPI_File_open(MPI_COMM_WORLD, out_file.c_str(), MPI_MODE_WRONLY,
                MPI_INFO_NULL, &fout);
...
#pragma omp parallel
  {
    ...
    printf("I am thread: %d @ rank: %d\n", thread_id, rank);
#pragma omp for
    for (int i = start_user; i < end_user; i++) {
      V<P<int, double>> topk = QueryHNSW(
          &user_buff[(i - start_user) * ((ll)embedding_size)], ep, indptr,
          index, level_offset, max_level, vect, K, num_news, embedding_size);
      for (int j = K; j > 0; j--) {
        std::pop_heap(topk.begin(), topk.begin() + j, comparePairMax());
        items[i - start_user][K - j] = topk[j - 1].first;
      }
    }
  }
  ...
  //   MPI_Win_create((void *)vect_buff, embedding_size * (end_news -
  //   start_news),
  //                  embedding_size * sizeof(double), MPI_INFO_NULL,
  ...
  //       V<P<int, double>> topk =
  //           QueryHNSW(user_buff, ep, indptr, index, level_offset,
  //           max_level,
